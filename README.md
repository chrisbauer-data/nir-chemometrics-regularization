...
Key result
Model	Test RMSE	Test R2	Predictors retained
LASSO	6.56	0.855	33 of 235
Ridge	11.77	0.532	235 of 235

LASSO substantially outperforms ridge on held-out data. With true signal concentrated in a handful of latent dimensions, LASSO's sparse-selection approach matches the underlying structure of the data more closely than ridge's uniform-shrinkage approach.
...
Originally developed as graduate coursework. Revised here with a proper held-out test set (the original evaluated LASSO on training data only) and extended with PCA and a ridge comparison to build a complete analytical narrative.
